...
def sample_pc(mesh_or_path, pc_num, with_normal=False):

    # The caller (Dataset) already hands us a loaded + normalized Trimesh.
    # Older trimesh let trimesh.load() pass a mesh through; trimesh>=4 raises,
    # so accept a mesh object directly and only load when given a path.
    if hasattr(mesh_or_path, "sample"):
        mesh = mesh_or_path
    else:
