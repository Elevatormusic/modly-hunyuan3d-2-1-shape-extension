...
Pipeline (all validated against the real 2.6M-face mesh, ~few seconds CPU):
  1. read the GLB's own accessor arrays (positions/normals/uvs/faces) — the one
     source of truth — and compute per-vertex UV-space tangents (Lengyel).
  2. rasterize UV atlas to per-texel {position, normal, tangent, handedness} +
     coverage mask; Open3D closest-point onto the dense mesh -> interpolate its
     SMOOTH vertex normals at the hit barycentric coords.
  3. transform dense normal into the per-texel UV-tangent frame
     (B = w * cross(N, T)), +Y encode, dilate seams.
  4. attach as glTF normalTexture (force NORMAL export), then ship the SAME
     per-vertex tangents as the glTF TANGENT accessor, so every compliant
     viewer decodes in exactly the basis we encoded in (spec RV-A..D).
"""
...
def read_glb_arrays(glb_path):
    ...
    if prim.attributes.TEXCOORD_0 is None:
        raise ValueError("GLB primitive lacks TEXCOORD_0 (no UVs)")
    positions = acc(prim.attributes.POSITION, 3, np.float32)
    if prim.attributes.NORMAL is not None:
        normals = acc(prim.attributes.NORMAL, 3, np.float32)
    else:
        # Paint/mr_export/seam_fix GLBs ship no NORMAL accessor (trimesh only
        # writes one when vertex normals were computed before export). The
        # final GLB gets trimesh's smooth vertex normals at
        # attach_normal_texture's re-export, so fall back to exactly those
        # (same vertices, same faces, process=False -> identical array;
        # encode basis == decode basis).
        import trimesh
        normals = np.asarray(
            trimesh.Trimesh(vertices=positions, faces=faces,
                            process=False).vertex_normals, np.float64)
    return dict(
        positions=positions,
        normals=normals,
...
def uv_to_texel(uv, size):
    """Accessor-space (glTF) UV -> float texel coords (col = u*(size-1),
    row = v*(size-1)); atlas array row 0 == PNG top row == glTF v=0, so no
    flip anywhere. This is the ONE uv->texel conversion shared by the
    rasterization, encode and export paths (spec RV-D: a V-flip bug must be
    impossible to write twice)."""
    return np.asarray(uv, np.float64) * (size - 1)


def rasterize_uv_atlas(verts, faces, uv, vertex_normals, size=2048,
                       vertex_tangents=None, tangent_w=None):
    """Barycentric-rasterize each triangle in UV space.

    UVs are accessor-space (glTF convention, v grows downward from the top
    image row); the uv->texel mapping goes through uv_to_texel — the same
    helper every other UV consumer uses. When vertex_tangents/tangent_w are
    given, they are interpolated per texel too and the return grows to
    (pos, nrm, mask, tan_map, w_map)."""
    ...
    want_tan = vertex_tangents is not None
    if want_tan:
        vt = np.asarray(vertex_tangents, np.float64)
        vw = np.asarray(tangent_w, np.float64)
        tan_map = np.zeros((size, size, 3), np.float32)
        w_map = np.ones((size, size), np.float32)

    for tri in faces:
        uva, uvb, uvc = uv_to_texel(uv[tri], size)
        ...
        if want_tan:
            ta, tb, tc = vt[tri]
            t = wa[:, None] * ta + wb[:, None] * tb + wc[:, None] * tc
            tan_map[pyi, pxi] = t.astype(np.float32)
            w_map[pyi, pxi] = (wa * vw[tri[0]] + wb * vw[tri[1]]
                               + wc * vw[tri[2]]).astype(np.float32)

    ln = np.linalg.norm(nrm, axis=2, keepdims=True)
    ln[ln == 0] = 1.0
    nrm = nrm / ln
    if want_tan:
        return pos, nrm, mask, tan_map, w_map
    return pos, nrm, mask
...
def encode_tangent_space(world_nrm, mask, tex_T, tex_w, tex_N):
    """Transform world-space dense normals into each texel's tangent frame (+Y encode).

    The frame is the interpolated per-vertex UV-tangent basis — the SAME basis
    exported as the glTF TANGENT accessor — so every compliant viewer decodes
    with exactly the basis we encoded in (spec RV-A/RV-C):
    T re-orthogonalized vs N per texel, B = w * cross(N, T).
    """
    N = tex_N
    T = tex_T - np.sum(tex_T * N, axis=2, keepdims=True) * N
    T = T / (np.linalg.norm(T, axis=2, keepdims=True) + 1e-9)
    w = np.where(tex_w[..., None] < 0.0, -1.0, 1.0)
    B = w * np.cross(N, T)
    nt = np.stack([
...
def bake_normal_map(dense, low_glb_path, size=2048):
    ...
    Encode basis == decode basis by construction: ONE array set — the GLB's own
    accessors (read_glb_arrays) — drives the tangent computation, the atlas
    rasterization (via uv_to_texel, the single UV convention), the per-texel
    encode, and the exported TANGENT attribute (spec RV-A..D).

    Returns True on success (writes <base>_normal.png, attaches normalTexture
    and the matching TANGENT accessor), False on any failure (leaves the
    textured GLB unchanged).
    """
    try:
        from PIL import Image
        try:
            arr = read_glb_arrays(low_glb_path)
        except ValueError as exc:
            print(f"[normal_bake] low mesh has no UVs ({exc}); skipping")
            return False
        verts, faces = arr["positions"], arr["faces"]
        uv, vnorm = arr["uvs"], arr["normals"]
        ln = np.linalg.norm(vnorm, axis=1, keepdims=True)
        ln[ln == 0] = 1.0
        vnorm = vnorm / ln
        tan, w = compute_uv_tangents(verts, vnorm, uv, faces)
        pos, low_nrm, mask, tex_T, tex_w = rasterize_uv_atlas(
            verts, faces, uv, vnorm, size,
            vertex_tangents=tan, tangent_w=w)
        world_nrm = sample_dense_normals(dense, pos, mask, low_nrm=low_nrm)
        rgb = encode_tangent_space(world_nrm, mask, tex_T, tex_w, low_nrm)
        rgb = dilate_map(rgb, mask)
        png = low_glb_path[:-4] + "_normal.png"
        Image.fromarray(rgb, "RGB").save(png)
        with open(low_glb_path, "rb") as fh:
            pre_bake_glb = fh.read()  # restored if the TANGENT export fails
        attach_normal_texture(low_glb_path, png)
        if not attach_tangents(low_glb_path, tan, w):
            # A normal map without its exported basis would resurrect the
            # viewer mismatch this bake exists to fix — fail honestly and
            # keep the "False leaves the GLB unchanged" contract.
            with open(low_glb_path, "wb") as fh:
                fh.write(pre_bake_glb)
            print("[normal_bake] TANGENT export failed; bake rolled back, "
                  "leaving textured GLB unchanged")
            return False
        print(f"[normal_bake] wrote {png} and attached normalTexture "
              f"basis=uv-tangent(exported)")
        return True
    except Exception as exc:
        import traceback
        traceback.print_exc()
